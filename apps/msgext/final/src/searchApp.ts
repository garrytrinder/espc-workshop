  ...
  public async handleTeamsMessagingExtensionQuery(
    ...
    return await handleMessageExtensionQueryWithSSO(
      ...
      async (token: MessageExtensionTokenResponse) => {
        ...
        const { sharepointIds } = await graphClient
          .api(`/sites/${config.spoHostname}:/${config.spoSiteUrl}`)
          .select("sharepointIds")
          .get();
        const { value: items } = await graphClient
          .api(
            `/sites/${
              sharepointIds.siteId
            }/lists/Products/items?expand=fields&select=${listFields.join(
              ","
            )}&$filter=startswith(fields/Title,'${query.parameters[0].value}')`
          )
          .get();

        // get the drives from the site and find the Product Imagery drive
        const { value: drives } = await graphClient
          .api(`sites/${sharepointIds.siteId}/drives`)
          .select(["id", "name"])
          .get();
        const { id: productImageryDriveId } = drives.find(
          (drive) => drive.name === "Product Imagery"
        );
        ...
        await Promise.all(
          items.map(async (item) => {
            ...
            const { id: driveItemId } = await graphClient
              .api(
                `sites/${sharepointIds.siteId}/drives/${productImageryDriveId}/root:/${fileName}`
              )
              .get();
            const { value: thumbnails } = await graphClient
              .api(
                `sites/${sharepointIds.siteId}/drives/${productImageryDriveId}/items/${driveItemId}/thumbnails`
              )
              .get();
            ...
            const cardImages: CardImage[] = [
              { url: thumbnails[0].small.url, alt: Title },
            ];
