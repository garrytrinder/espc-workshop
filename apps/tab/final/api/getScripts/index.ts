...
export default async function run(
  ...
  try {
    ...
    console.log(sharepointIds);
    const token = await authProvider.getAccessToken();
    console.log(token);
    const { value: items } = await graphClient.api(`/sites/${sharepointIds.siteId}/lists/Scripts/items?expand=fields`).get();
    console.log(items);
    // get the drives from the site and find the Scripts drive
    const { value: drives } = await graphClient.api(`sites/${sharepointIds.siteId}/drives`).select(["id", "name"]).get();
    const { id: scriptsDriveId } = drives.find(drive => drive.name === "Scripts");

    // iterate items and get a custom sized thumbnail image 
    const result = await Promise.all(items.map(async (item) => {
        const { id: driveItemId } = await graphClient.api(`sites/${sharepointIds.siteId}/drives/${scriptsDriveId}/root:/${item.fields.FileLeafRef}`).get();
        const { value: thumbnails } = await graphClient.api(`sites/${sharepointIds.siteId}/drives/${scriptsDriveId}/items/${driveItemId}/thumbnails?select=c350x350_crop`).get();
    ...
    return {
      ...
      body: {
      error:
          "Failed to retrieve scripts from Microsoft Graph. The application may not be authorized.",
      },
  };
  }
